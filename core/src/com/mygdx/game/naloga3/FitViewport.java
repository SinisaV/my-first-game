package com.mygdx.game.naloga3;
...
public class FitViewport extends ApplicationAdapter {
    SpriteBatch batch;
    ...
    public void create () {
        ...
        viewport=new com.badlogic.gdx.utils.viewport.FitViewport(500,400,camera);
        ...
        backpack.x = viewport.getWorldWidth() / 2 - backpackImg.getWidth() / 2f;
        backpack.y = 20f;
    ...
    private void update(float delta) {
        ...
        for (Rectangle bullet : bullets) {
            ...
            if (bullet.y > viewport.getWorldHeight()) {
                bullets.removeValue(bullet, true);
            }
        }
    ...
    private void draw() {
        ...
        if (health <= 0) {
            ...
            font.draw(batch,
                    "GAME OVER",
                    20f, viewport.getWorldHeight() - 20
            );
            font.setColor(Color.NAVY);
            font.draw(batch,
                    "YOUR SCORE: " + dumbbellsCollected,
                    20f, viewport.getWorldHeight() - 60
            );
            return;
        }
        ...
        font.draw(batch,
                ...
                20f, viewport.getWorldHeight() - 100
        );
    }
    ...
    private void moveRight(float delta) {
        backpack.x += BACKPACK_SPEED * delta;
        if (backpack.x > viewport.getWorldWidth() - backpackImg.getWidth())
            backpack.x = viewport.getWorldWidth() - backpackImg.getWidth();
    }

    private void spawnDumbbell() {
        Rectangle dumbbell = new Rectangle();
        dumbbell.x = MathUtils.random(0f, viewport.getWorldWidth() - backpackImg.getWidth());
        dumbbell.y = viewport.getWorldHeight();
    ...
    private void spawnPizza() {
        ...
        pizza.x = MathUtils.random(0f, viewport.getWorldWidth() - pizzaImg.getWidth());
        pizza.y = viewport.getWorldHeight();
